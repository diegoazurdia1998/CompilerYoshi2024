...
namespace ProyectoCompiladores.Crziness
{
    public class lalrLecter
    {
        ...
        public LR1Table ReadLR1Table(string filePath)
        {
            ...
            foreach (var line in File.ReadLines(filePath))
            {
                ...
                else if (!string.IsNullOrEmpty(trimmedLine))
                {
                    ...
                    if (match.Success)
                    {
                        int productionId = int.Parse(match.Groups[1].Value);
                        ...
                        if(currentItem.Equals(""))
                        {
                            ...
                            {
                                int reduceIndex = Grammar.Productions.IndexOf(prod);
                ...
                switch (action.Type)
                {
                    ...
                    case "R": // Reduce
                        ...
                        for (int i = 0; i < popCount; i++)
                        {
                            ...
                            {
                                symbolsToReduce.Add(symbolStack.Pop()); // Desapilar los símbolos leídos
                            }
                        }
        ...
        {
            // Aquí implementamos la lógica para buscar la acción en la lista de acciones
            return Actions.FirstOrDefault(a => a.Type != "R" && a.NextState == state && a.Symbol == token);
        }
